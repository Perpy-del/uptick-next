...
const EditBlogFormComponent = ({ params }: { params: { blogId: string } }) => {
  const router = useRouter();
  const { editBlogForm, editPostSubmit, blogPost, newBlogLoading, getBlog } = useUptickHook();
  ...
  }, [blogPost, editBlogForm]);

  return (
    <Form {...editBlogForm}>
      <form
        onSubmit={editBlogForm.handleSubmit((values: any) => editPostSubmit(values, params.blogId))}
        className="space-y-2"
      >
        <FormField
      ...
      <Button variant={'destructive'} className="w-64" onClick={() => router.push('/blog')}>Cancel</Button>
    </Form>
  );
};
